...
void check_kprobe(Probe &p, const std::string &attach_point,
                  const std::string &orig_name, uint64_t func_offset = 0)
{
...
TEST(bpftrace, add_probes_offset)
{
  ...
  check_kprobe(bpftrace.get_probes().at(0), "sys_read", probe_orig_name, offset);
}
