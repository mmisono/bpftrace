...
std::string AttachedProbe::eventname() const
{
  ...
  switch (probe_.type)
  {
    ...
    case ProbeType::kretprobe:
      ...
      return eventprefix() + sanitise(probe_.attach_point) + "_" + offset_str.str() + index_str;
    case ProbeType::uprobe:
    case ProbeType::uretprobe:
    case ProbeType::usdt:
      offset_str << std::hex << offset_;
...
static void check_alignment(std::string& path, std::string& symbol, uint64_t sym_offset,
                            uint64_t func_offset, bool safe_mode, ProbeType type)
{
  Disasm dasm(path);
  ...
  switch (aligned)
  {
    case AlignState::NotAlign:
      if (safe_mode)
        throw std::runtime_error("Could not add " + probe_name + " into middle of instruction: " + tmp);
      else
        std::cerr << "Unsafe " + probe_name + " in the middle of the instruction: " << tmp << std::endl;
      break;

     case AlignState::Fail:
       if (safe_mode)
         throw std::runtime_error("Failed to check if " + probe_name + " is in proper place: " + tmp);
       else
     ...
     case AlignState::NotSupp:
       if (safe_mode)
         throw std::runtime_error("Can't check if " + probe_name + " is in proper place (compiled without (k|u)probe offset support): " + tmp);
       else
         std::cerr << "Unchecked " + probe_name + " : " << tmp << std::endl;
       break;
...
void AttachedProbe::resolve_offset_uprobe(bool safe_mode)
{
  ...
  check_alignment(probe_.path, symbol, sym_offset, func_offset, safe_mode, probe_.type);
}
...
static std::string find_vmlinux()
{
  ...
  throw std::runtime_error("Could not find vmlinux");
}
...
void AttachedProbe::resolve_offset_kprobe(bool safe_mode)
{
  ...
  check_alignment(path, symbol, sym_offset, func_offset, safe_mode, probe_.type);
}
...
void AttachedProbe::attach_kprobe(bool safe_mode)
{
  ...
  int perf_event_fd = cast_signature<attach_probe_wrapper_signature>(&bpf_attach_kprobe)(progfd_, attachtype(probe_.type),
      eventname().c_str(), probe_.attach_point.c_str(), offset_, 0);
